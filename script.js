...
document.getElementById("toggleMusicBtm").style.transform = "rotate(180deg)";
...
function addEventListeners(element, id, index) {
  element.addEventListener("mouseenter", function () {
    ...
    setTimeout(function () {
      if (
        document.getElementById("music_list_" + index).offsetHeight === 182 &&
        audio.paused
      ) {
...
function fadeChours(inOrOut) {
  if (inOrOut == "in") {
    ...
    let totalTime = 1000;
    let interval = 100;
    let volumeIncreasement = (targetVolume - volume) / (totalTime / interval);
  ...
  if (inOrOut == "out") {
    ...
    let totalTime = 1000;
    let interval = 100;
    let volumeDecreasement = (targetVolume - volume) / (totalTime / interval);
    let fadeInterval = setInterval(function () {
      if (isPlayingChorus && isFading) {
  ...
  try {
    ...
    data.forEach((item, index) => {
      ...
      translateButton.addEventListener("click", function () {
        arr_isShowDetalisTranslate[index] = !arr_isShowDetalisTranslate[index];
        if (arr_isShowDetalisTranslate[index]) {
...
function loadMusicInfo(id) {
  ...
  document.getElementById("list_item_" + nowPlayingMusicId).style.fontWeight =
    "normal";
  ...
  document.getElementById("toggleMusic").style.backgroundImage =
    "url(" + cover + ")";
  document.getElementById("music_box_info_title").innerHTML = data[id].title;
  document.getElementById("music_box_info_author").innerHTML = data[id].author;
  document.getElementById("music_box_info_album").innerHTML = data[id].album;
  ...
  volumeTrack.style.backgroundColor = hexToRgba(data[id].mainColor, false, 0.2);
...
document.getElementById("loop").addEventListener("click", toggleChangePlayMode);
document
...
function updateProgressAndAudio(e) {
  ...
  if (
    ...
    playState.innerHTML = s_To_mmss(Math.round(audio.duration * percentage));
...
document.getElementById("progress").addEventListener("mouseup", handleMouseUp);
document
  .getElementById("progress")
...
function changeCurrentLyrics() {
  ...
  // 添加当前歌词的 ly_now 类
  if (currentIndex !== -1) {
    const currentLyricElement = document.getElementById(`ly_${currentIndex}`);
...
async function fetchTranslatedLyrics(id) {
  if (data[id].isTranslate === "0") {
    ...
    try {
      ...
      console.error(`Failed to fetch translated lyrics for id ${id}:`, error);
      return [];
    }
  }
}
